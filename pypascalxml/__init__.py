...
class PascalVOC_XML_Annotation(object):
    ...
    def xml(an):
        template = open('template_annotation.xml', 'r')
        ...
        template = template.replace('_^_OBJECT_MULTIPLE_^_', '\n'.join(objects))
...
class PascalVOC_XML_Object(object):
    ...
    def xml(ob):
        template = open('template_object.xml', 'r')
        template = ''.join(template.readlines())
        ...
        template = template.replace('_^_PART_MULTIPLE_OPTIONAL_^_', '\n'.join(parts))
...
class PascalVOC_XML_Part(object):
    ...
    def xml(pt):
        template = open('template_part.xml', 'r')
        template = ''.join(template.readlines())
